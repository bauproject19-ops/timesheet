    ...
    for uid in REMINDER_USER_IDS:
        ...
        if scold:
            msg += (
                ...
                f"(выбери нужную дату в поле \"Дата\" формы): {FORM_URL}\n\n"
            )
        if day_off_today:
            msg += (
                f"Сегодня {today.strftime('%d.%m.%Y')} — выходной/праздник. Норма 0ч, "
                f"заполнять не обязательно. Если всё же работал(а) сегодня — занеси часы: {FORM_URL}"
            )
        else:
            msg += (
                f"Не забудь заполнить часы за {today.strftime('%d.%m.%Y')} "
                f"(рабочий день, норма 8ч). Форма: {FORM_URL}"
            )
